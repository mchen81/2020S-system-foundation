...
// projcet7
int psinfo(struct pitable *pit){
    struct proc *p;
    struct pinfo *pi;
    int count = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
        if(p->state != UNUSED && p->state !=SUSPEND) {
            pi = &pit->pinfos[count];
            pi->pid  = p->pid;
            pi->mem  = p->sz;
...
struct proc* suspend(int pid){
    
    struct proc *p;
    acquire(&ptable.lock);
    
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
        if(p->pid == pid && p->state != UNUSED && p->state != SUSPEND){
            p->state = SUSPEND;
            release(&ptable.lock);
            return p;
        }
...
int fork(void)
{
    ...
    *np->tf = *proc->tf;
    ...
    // project7-part1 initialize tracing
    np->tracing = 0;
